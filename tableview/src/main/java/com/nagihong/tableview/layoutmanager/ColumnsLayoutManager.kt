package com.nagihong.tableview.layoutmanager
...
class ColumnsLayoutManager : Serializable {
    ...
    /**
     * 用于在非主线程预先Measure/Layout内容，减轻实际展示内容时的工作量
     * 对于 [DrawableColumn] 其内容是可直接Measure/Layout的
     * 对于 非[DrawableColumn] 直接使用column本身提供的尺寸
     */
        ...
        for (index in 0 until maxSize) {
            ...
            if (!column.visible()) {
                ...
                if (column.widthWithMargins > 0 && column.heightWithMargins > 0) {
                    // 记录maxHeight，由于此函数属于热点代码，节省从visibleColumnsHeightWithMargins遍历得到的开销
